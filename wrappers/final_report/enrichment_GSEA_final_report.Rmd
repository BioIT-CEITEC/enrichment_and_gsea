...
`r if(config_json$gseapy_enrichr[1]==TRUE) paste0(" - enrichR - ",config_json$enrichR_DB[1])`
...
- **EnrichR** annotations are provided by the tool **gseapy** and the enrichR library: **`r config_json$enrichR_DB[1]`**.
...
if(config_json$onthology[1]==TRUE){
  BP.files <- data.table(tablpath = list.files(path = DE, pattern = "enrich_GO_BP.tsv", full.names = T, recursive = T))
...
if(config_json$onthology[1]==TRUE){
  MF.files <- data.table(tablpath = list.files(path = DE, pattern = "enrich_GO_MF.tsv", full.names = T, recursive = T))
...
if(config_json$onthology[1]==TRUE){
  CC.files <- data.table(tablpath = list.files(path = DE, pattern = "enrich_GO_CC.tsv", full.names = T, recursive = T))
...
if(config_json$kegg[1]==TRUE){
  ke.files <- data.table(tablpath = list.files(path = DE, pattern = "enrich_KEGG.tsv", full.names = TRUE, recursive = TRUE))
...
if(config_json$reactome[1]==TRUE){
  re.files <- data.table(tablpath = list.files(path = DE, pattern = "enrich_REACTOME.tsv", full.names = TRUE, recursive = TRUE))
...
if(config_json$wikipathways[1]==TRUE){
  wp.files <- data.table(tablpath = list.files(path = DE, pattern = "enrich_WP.tsv", full.names = TRUE, recursive = TRUE))
...
- Main output files are **enrich_xxx.tsv**, **GSEA_xxx.tsv** where xxx correspond to the source of annotations. Plots to display the results are provided in pdf and svg formats.
...
`r if(config_json$onthology[1]==TRUE) paste0("- **enrichment_GO_{all|up|down}/enrich_GO_{BP|MF|CC}.tsv**: TAB separated enrichment results table for Gene Onthology - Biological Process - Molecular Function - Cellular Component.")`
`r if(config_json$onthology[1]==TRUE) paste0("- **enrichment_GO_{all|up|down}/enrich_GO_{BP|MF|CC}.{pdf|svg}**: Plots for enrichment results for Gene Onthology - Biological Process - Molecular Function - Cellular Component.")`
`r if(config_json$onthology[1]==TRUE) paste0("- **GSEA_GO_{all|up|down}/GSEA_GO_{BP|MF|CC}.tsv**: TAB separated GSEA results table for Gene Onthology - Biological Process - Molecular Function - Cellular Component.")`
`r if(config_json$onthology[1]==TRUE) paste0("- **GSEA_GO_{all|up|down}/GSEA_GO_{BP|MF|CC}.{pdf|svg}**: Plots for GSEA results for Gene Onthology - Biological Process - Molecular Function - Cellular Component.")`
`r if(config_json$kegg[1]==TRUE) paste0("- **enrichment_KEGG_{all|up|down}/enrich_KEGG.tsv**: TAB separated enrichment results table for KEGG pathways.")`
`r if(config_json$kegg[1]==TRUE) paste0("- **enrichment_KEGG_{all|up|down}/enrich_KEGG.{pdf|svg}**: Plots for enrichment results for KEGG pathways.")`
`r if(config_json$kegg[1]==TRUE) paste0("- **GSEA_KEGG_{all|up|down}/GSEA_KEGG.tsv**: TAB separated GSEA results table for KEGG pathways.")`
`r if(config_json$kegg[1]==TRUE) paste0("- **GSEA_KEGG_{all|up|down}/GSEA_KEGG.{pdf|svg}**: Plots for GSEA results for KEGG pathways.")`
`r if(config_json$reactome[1]==TRUE) paste0("- **enrichment_REACTOME_{all|up|down}/enrich_REACTOME.tsv**: TAB separated enrichment results table for REACTOME pathways.")`
`r if(config_json$reactome[1]==TRUE) paste0("- **enrichment_REACTOME_{all|up|down}/enrich_REACTOME.{pdf|svg}**: Plots for enrichment results for REACTOME pathways.")`
`r if(config_json$reactome[1]==TRUE) paste0("- **GSEA_REACTOME_{all|up|down}/GSEA_REACTOME.tsv**: TAB separated GSEA results table for REACTOME pathways.")`
`r if(config_json$reactome[1]==TRUE) paste0("- **GSEA_REACTOME_{all|up|down}/GSEA_REACTOME.{pdf|svg}**: Plots for GSEA results for REACTOME pathways.")`
`r if(config_json$wikipathways[1]==TRUE) paste0("- **enrichment_WP_{all|up|down}/enrich_WP.tsv**: TAB separated enrichment results table for WikiPathways.")`
`r if(config_json$wikipathways[1]==TRUE) paste0("- **enrichment_WP_{all|up|down}/enrich_WP.{pdf|svg}**: Plots for enrichment results for WikiPathways.")`
`r if(config_json$wikipathways[1]==TRUE) paste0("- **GSEA_WP_{all|up|down}/GSEA_WP.tsv**: TAB separated GSEA results table for WikiPathways.")`
`r if(config_json$wikipathways[1]==TRUE) paste0("- **GSEA_WP_{all|up|down}/GSEA_WP.{pdf|svg}**: Plots for GSEA results for WikiPathways.")`
`r if(config_json$gseapy_enrichr[1]==TRUE) paste0("- **enrichr_{all|up|down}/",config_json$enrichR_DB[1],".",config_json$organism_enrichR[1],".enrichr.reports.txt**: TAB separated enrichment results table for EnrichR.")`
`r if(config_json$gseapy_enrichr[1]==TRUE) paste0("- **enrichr_{all|up|down}/",config_json$enrichR_DB[1],".",config_json$organism_enrichR[1],".enrichr.reports.svg**: Plots for enrichment results for EnrichR.")`
